...
gulp.task('start', (cb) => {
  ...
  console.log('==== antd-mobile 里与 fashop-components 下的同名组件将被覆盖 ======');
  console.log('====================================================================');
  syncAnt.init(cb);
});
gulp.task('default', ['start']);

gulp.watch('fashop-components/**/*', (event) => {
  console.log(`File ${event.path} was ${event.type} running tasks...`);
  // syncAnt.syncBiz();
  // 只同步改变的文件
  fs.copySync(event.path, event.path.replace('/fashop-components/', '/components/'));
});
